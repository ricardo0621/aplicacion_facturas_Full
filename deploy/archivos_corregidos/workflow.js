 ...
 * @param {string} [rolAprobadorRuta2] - (Opcional) Rol de aprobador de RUTA_2 seleccionado en la factura
 * @returns {Object} - { nuevoEstado, esRechazo, estadoRetorno? }
 */
const calcularTransicion = (estadoActualCodigo, accion, estadoRetornoCorreccion = null, estadoRuta2Aprobador = null, rolAprobadorRuta2 = null) => {
    ...
    if (accion === ACCIONES.CORREGIR) {
        ...
        if (!estadoRetornoCorreccion) {
            // Si no hay estado de retorno, mapear según rol de aprobador
            ...
            // Si no hay rol o no se pudo mapear, usar RUTA_2 por defecto
    ...
    if (accion === 'ENVIAR_REVISION') {
        ...
        // Si no, mapear según rol de aprobador seleccionado
        if (rolAprobadorRuta2) {
            const estadoMapeado = mapearRolAEstado(rolAprobadorRuta2);
            if (estadoMapeado) {
                return { nuevoEstado: estadoMapeado, esRechazo: false };

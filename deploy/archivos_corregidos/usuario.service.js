...
const crearUsuario = async (datosUsuario) => {
    const { nombre, email, password, tipo_documento, numero_documento, area, cargo } = datosUsuario;
    ...
    try {
        ...
        const insertQuery = `
            ...
            RETURNING usuario_id, nombre, email, tipo_documento, numero_documento, area, cargo, activo, fecha_creacion
        `;
        const result = await client.query(insertQuery, [nombre, email, password_hash, tipo_documento, numero_documento, area, cargo]);
...
const listarUsuarios = async () => {
    const client = await db.connect();
    try {
        // Sintaxis compatible con PostgreSQL 9.2
        // Filtramos NULL en JavaScript porque array_remove no existe en PG 9.2
        const query = `
            SELECT 
                u.usuario_id, u.nombre, u.email, u.tipo_documento, u.numero_documento, u.area, u.cargo, u.activo, 
                u.fecha_creacion,
                ARRAY_AGG(r.nombre) as roles,
                ARRAY_AGG(r.codigo) as roles_codigos
            FROM usuarios u
        ...
        // Filtrar valores NULL de los arrays en JavaScript
        return result.rows.map(row => ({
            ...row,
            roles: row.roles ? row.roles.filter(r => r !== null) : [],
            roles_codigos: row.roles_codigos ? row.roles_codigos.filter(r => r !== null) : []
        }));
    } finally {
        client.release();
    }
    ...
    try {
        ...
        const query = `
            SELECT 
                u.usuario_id, u.nombre, u.email, u.tipo_documento, u.numero_documento, u.area, u.cargo, u.activo, 
                u.fecha_creacion,
                ARRAY_AGG(r.rol_id) as roles_ids,
                ARRAY_AGG(r.nombre) as roles,
                ARRAY_AGG(r.codigo) as roles_codigos
            FROM usuarios u
            LEFT JOIN usuario_roles ur ON u.usuario_id = ur.usuario_id
        ...
        return {
            ...usuario,
            roles_ids: usuario.roles_ids ? usuario.roles_ids.filter(r => r !== null) : [],
            roles: usuario.roles ? usuario.roles.filter(r => r !== null) : [],
            roles_codigos: usuario.roles_codigos ? usuario.roles_codigos.filter(r => r !== null) : []
        };
    } finally {
        client.release();
    }
...
const actualizarUsuario = async (usuarioId, datos) => {
    const { nombre, email, password, tipo_documento, numero_documento, area, cargo } = datos;
    const client = await db.connect();
    ...
    try {
        ...
        const updateQuery = `
            ...
            RETURNING usuario_id, nombre, email, tipo_documento, numero_documento, area, cargo, activo
        `;
...
const desactivarUsuario = async (usuarioId) => {
    ...
    try {
        const result = await client.query(
            'UPDATE usuarios SET activo = FALSE WHERE usuario_id = $1 RETURNING usuario_id, nombre, activo',
            [usuarioId]
        );
...
const activarUsuario = async (usuarioId) => {
    ...
    try {
        const result = await client.query(
            'UPDATE usuarios SET activo = TRUE WHERE usuario_id = $1 RETURNING usuario_id, nombre, activo',
            [usuarioId]
        );
...
const obtenerPermisosUsuario = async (usuarioId) => {
    ...
    try {
        ...
        // Filtrar valores NULL del array de roles
        const permisos = result.rows[0];
        return {
            ...permisos,
            roles: permisos.roles ? permisos.roles.filter(r => r !== null) : []
        };
    } finally {
        client.release();
    }

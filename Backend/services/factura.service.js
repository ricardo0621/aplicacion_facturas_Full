...
const crearFactura = async (facturaData, file, userId) => {
    ...
    const UPLOAD_DIR = process.env.SOPORTES_PATH || path.join(__dirname, '..', 'uploads');
...
const crearFacturaConMultiplesArchivos = async (facturaData, files, tiposDocumento, userId, rolAprobadorRuta2 = null) => {
    ...
    try {
        ...
        const insertQuery = `
            INSERT INTO facturas (
                numero_factura, proveedor_id, fecha_emision, monto, concepto,
                estado_id, usuario_creacion_id, documento_nombre,
                rol_aprobador_ruta2, nit_proveedor
            )
...
const eliminarDocumento = async (documentoId, userId) => {
    ...
    try {
        ...
        const UPLOAD_DIR = process.env.SOPORTES_PATH || path.join(__dirname, '..', 'uploads');
        const fullPath = path.join(UPLOAD_DIR, doc.ruta_archivo);
    ...
    try {
        ...
        if (files.documento) {
            ...
            if (docActualRes.rows.length > 0) {
                ...
                try {
                    const UPLOAD_DIR = process.env.SOPORTES_PATH || path.join(__dirname, '..', 'uploads');
                    const fullPath = path.join(UPLOAD_DIR, docAnterior.ruta_archivo);
                    if (fs.existsSync(fullPath)) {
                        fs.unlinkSync(fullPath);
                    }
                } catch (err) {
            ...
            `, [facturaId, 'FACTURA', files.documento.filename, files.documento.filename]);
        ...
        if (documentosEliminar && documentosEliminar.length > 0) {
            ...
            for (const docId of documentosEliminar) {
                ...
                if (docRes.rows.length > 0) {
                    ...
                    try {
                        const UPLOAD_DIR = process.env.SOPORTES_PATH || path.join(__dirname, '..', 'uploads');
                        const fullPath = path.join(UPLOAD_DIR, rutaArchivo);
                        if (fs.existsSync(fullPath)) {
                            fs.unlinkSync(fullPath);
                        }
                    } catch (err) {
